...
def check_expiry(expiry_date):
    ...
    return "Expired" if localized_expiry_date_for_check < current_nz_time else "Active"
...
def geocode_address(address):
    ...
    try:
        ...
        if location:
            ...
            print(f"DEBUG: Geocoding failed for '{standardized_address}' (no location found).")
            return (None, None)
...
def extract_metadata(text):
    ...
    for item in rc_matches:
        if isinstance(item, tuple):
            flattened_rc_matches.append(item[-1].strip()) # strip whitespace
        else:
            flattened_rc_matches.append(item.strip()) # strip whitespace
    rc_str = ", ".join(list(dict.fromkeys(flattened_rc_matches)))

    # Company name patterns
    company_patterns = [
        r"Applicant:\s*(.+?)(?=\s*Site address)", # Use lookahead for clean capture
        r"Applicant's name:\s*(.+?)(?=\s*Site address)" # Use lookahead for clean capture
    ]

    company_matches = []
    for pattern in company_patterns:
        company_matches.extend(re.findall(pattern, text, re.IGNORECASE))
    company_str = ", ".join(list(dict.fromkeys([c.strip() for c in company_matches]))) # strip whitespace

    # Address patterns
    address_pattern = r"Site address:\s*(.+?)(?=\s*Legal description|\s*Legal)" # Add optional 'description' to Legal
    address_match = re.findall(address_pattern, text, re.IGNORECASE)
    address_str = ", ".join(list(dict.fromkeys([a.strip() for a in address_match]))) # strip whitespace
    ...
    issue_date_patterns = [
        ...
        r"(\b\d{2}/\d{2}/\d{2}\b)" # Correctly added this pattern here.
    ]
    
    issue_date = None
    for pattern in issue_date_patterns: # Iterate through the combined list
    ...
    return {
        ...
        "Consent Conditions Numbers": ", ".join(list(dict.fromkeys([re.match(r'^(\d+\.?\d*)', cond).group(1) for cond in full_conditions_list if re.match(r'^(\d+\.?\d*)', cond)]))) if full_conditions_list else "None",
        "Consent Conditions": "\n\n".join(full_conditions_list) if full_conditions_list else "None", # Join full conditions with double newline
    ...
    if all_data:
        ...
        with st.expander("Semantic Search Results", expanded=True):
            if query_input:
                ...
                corpus_embeddings = get_corpus_embeddings(tuple(corpus), model_name) # Pass tuple and model_name
            ...
            with st.spinner("AI is thinking..."):
                try:
                    ...
                    if not df.empty:
                        ...
                        context_sample_list = [{"Company Name": "Default Sample Ltd", "Resource Consent Numbers": "DIS60327400", "Address": "123 Default St, Auckland", "Consent Status": "Active", "AUP(OP) Triggers": "E14.1.1 (default)", "Mitigation (Consent Conditions)": "General Management Plan", "Issue Date": "2024-01-01", "Expiry Date": "2025-12-31", "Reason for Consent": "General default operations", "Consent Conditions Numbers": "1, 2, 3", "Consent Conditions": "1. Default condition A\n2. Default condition B"}]
