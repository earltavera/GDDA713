...
pdf_folder_input = st.sidebar.text_input("Enter path to folder containing PDFs:", value="uploaded_pdfs")
...
if not os.path.isdir(pdf_folder_input):
    st.sidebar.error(f"Folder not found: {pdf_folder_input}")
    st.stop()

pdf_file_paths = list_pdf_files(pdf_folder_input, recursive=recursive)
